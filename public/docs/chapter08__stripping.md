...
Um comando strip com a opção --strip-unneeded remove todos os símbolos de depuração de um binário ou biblioteca. Ele também remove todas as entradas da tabela de símbolos que não são normalmente necessárias pelo linker (para bibliotecas estáticas) ou linker dinâmico (para binários e bibliotecas compartilhadas vinculados dinamicamente). Usar --strip-debug não remove entradas da tabela de símbolos que podem ser necessárias para algumas aplicações. A diferença entre unneeded e debug é muito pequena. Por exemplo, um libc.a não stripado é de 22.4 MB. Após o strip com --strip-debug, ele tem 5.9 MB. Usar --strip-unneeded apenas reduz o tamanho ainda mais para 5.8 MB.

Os símbolos de depuração de bibliotecas selecionadas são compactados com Zstd e preservados em arquivos separados. Essa informação de depuração é necessária para executar testes de regressão com [ valgrind](https://www.linuxfromscratch.org/blfs/view/12.4-systemd//general/valgrind.html) ou [ gdb](https://www.linuxfromscratch.org/blfs/view/12.4-systemd//general/gdb.html) mais tarde, no BLFS.

Observe que o strip irá sobrescrever o arquivo binário ou de biblioteca que está processando. Isso pode travar os processos que usam código ou dados do arquivo. Se o processo que executa o strip for afetado, o binário ou a biblioteca que está sendo stripado pode ser destruído; isso pode tornar o sistema completamente inutilizável. Para evitar esse problema, copiamos algumas bibliotecas e binários para /tmp, fazemos o strip lá e depois os reinstalamos com o comando install. (A entrada relacionada em [Seção 8.2.1, “Problemas de Atualização”](#/page/chapter08__pkgmgt) fornece a justificativa para usar o comando install aqui.)
