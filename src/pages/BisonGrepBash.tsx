import { PageContainer } from "@/components/layout/PageContainer";
import { CodeBlock } from "@/components/ui/CodeBlock";

export default function BisonGrepBash() {
  return (
    <PageContainer
      title="Bison, Grep, Bash, Libtool"
      subtitle="Versões finais desses pacotes, agora linkadas à Glibc final."
      difficulty="intermediario"
      timeToRead="5 min"
    >
      <h2>Bison</h2>
      <CodeBlock
        language="bash"
        code={`cd /sources
tar -xf bison-3.8.2.tar.xz && cd bison-3.8.2
./configure --prefix=/usr --docdir=/usr/share/doc/bison-3.8.2
make && make install
cd .. && rm -rf bison-3.8.2`}
      />

      <h2>Grep</h2>
      <CodeBlock
        language="bash"
        code={`tar -xf grep-3.11.tar.xz && cd grep-3.11
sed -i "s/echo/#echo/" src/egrep.sh
./configure --prefix=/usr
make && make install
cd .. && rm -rf grep-3.11`}
      />

      <h2>Bash (final)</h2>
      <CodeBlock
        language="bash"
        code={`tar -xf bash-5.2.32.tar.gz && cd bash-5.2.32

./configure --prefix=/usr        \\
            --without-bash-malloc \\
            --with-installed-readline \\
            --docdir=/usr/share/doc/bash-5.2.32

make
chown -Rv tester .
su -s /usr/bin/expect tester << "EOF"
set timeout -1
spawn make tests
expect eof
lassign [wait] _ _ _ value
exit $value
EOF

make install
exec /usr/bin/bash --login
cd /sources && rm -rf bash-5.2.32`}
      />

      <h2>Libtool</h2>
      <CodeBlock
        language="bash"
        code={`tar -xf libtool-2.4.7.tar.xz && cd libtool-2.4.7
./configure --prefix=/usr
make && make install
rm -fv /usr/lib/libltdl.a
cd .. && rm -rf libtool-2.4.7`}
      />
    </PageContainer>
  );
}
